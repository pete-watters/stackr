...
function rpcRequest(body: unknown, headers: Record<string, string> = {}): Request {
  return new Request('https://stackr.ie/api/rpc/solana', {
    method: 'POST',
    headers: {
...
const getBalanceCall = { jsonrpc: '2.0', id: 1, method: 'getBalance', params: ['abc'] };

describe('POST /api/rpc/solana', () => {
  beforeEach(() => {
    vi.stubGlobal(
      'fetch',
      vi.fn(async () => Response.json({ jsonrpc: '2.0', id: 1, result: { value: 0 } })),
    );
  });
  ...
  it('forwards an allow-listed read method upstream', async () => {
    const res = await POST(rpcRequest(getBalanceCall));
  ...
  it('uses the Helius upstream when a server key is set, without echoing it', async () => {
    vi.stubEnv('HELIUS_API_KEY', 'secret-key-123');

    const res = await POST(rpcRequest(getBalanceCall));
    const text = await res.text();

    expect(res.headers.get('x-stackr-rpc-upstream')).toBe('helius');
    expect(text).not.toContain('secret-key-123');
    const upstreamUrl = vi.mocked(fetch).mock.calls[0]?.[0];
    expect(String(upstreamUrl)).toContain('helius');
  });

  it('refuses methods outside the read-only allow-list', async () => {
    const res = await POST(
      rpcRequest({ jsonrpc: '2.0', id: 1, method: 'sendTransaction', params: [] }),
    );
  ...
  it('refuses cross-origin requests', async () => {
    const res = await POST(rpcRequest(getBalanceCall, { origin: 'https://evil.example' }));
  ...
  it('allows same-origin requests with an Origin header', async () => {
    const res = await POST(rpcRequest(getBalanceCall, { origin: 'https://stackr.ie' }));

    expect(res.status).toBe(200);
  });

  it('rejects requests that omit Sec-Fetch-Site', async () => {
    const res = await POST(rawRpcRequest(getBalanceCall, { 'cf-connecting-ip': '203.0.113.10' }));
  ...
  it('fails closed when no client IP can be resolved', async () => {
    const res = await POST(rawRpcRequest(getBalanceCall, { 'sec-fetch-site': 'same-origin' }));
  ...
  it('never interpolates the api key into the upstream URL unencoded', async () => {
    ...
    await POST(rpcRequest(getBalanceCall));
  ...
  it('does not forward upstream HTTP error bodies verbatim', async () => {
    ...
    vi.stubGlobal(
      'fetch',
      vi.fn(async () => new Response('error for url ?api-key=secret-key-123', { status: 500 })),
    );
