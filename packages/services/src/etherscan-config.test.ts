...
describe('resolveEtherscanBase', () => {
  it('targets the same-origin proxy path in the browser', () => {
    ...
    expect(resolveEtherscanBase(true)).toBe(ETHERSCAN_PROXY_PATH);
  });
  ...
  it('falls back to the public base outside the browser (SSR/tests/server)', () => {
    ...
    expect(resolveEtherscanBase(false)).toBe(etherscanPublicBase);
  });

  it('keeps the public fallback an absolute URL fetch can consume', () => {
    expect(() => new URL(etherscanPublicBase)).not.toThrow();
  });
});
