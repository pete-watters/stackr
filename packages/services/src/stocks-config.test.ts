...
import { STOCKS_PROXY_PATH, stocksPublicBase, resolveStocksBase } from './stocks-config.js';

describe('resolveStocksBase', () => {
  it('targets the same-origin proxy path in the browser', () => {
    ...
    expect(resolveStocksBase(true)).toBe(STOCKS_PROXY_PATH);
  });
  ...
  it('falls back to the public base outside the browser (SSR/tests/server)', () => {
    ...
    expect(resolveStocksBase(false)).toBe(stocksPublicBase);
  });

  it('keeps the public fallback an absolute URL fetch can consume', () => {
    expect(() => new URL(stocksPublicBase)).not.toThrow();
  });
});
